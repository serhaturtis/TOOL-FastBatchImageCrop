...
TAGDATA_TEMPLATE = {"class_name": "", "tags": []}


class TagEditorTab(tk.Frame, RectangleMixin):
    def __init__(self, console):
        super().__init__()
        self.console = console
        self.input_path_entry = None
        self.class_name_entry = None
        self.image_tags_entry = None
    ...
    def init_ui(self):
        ...
        self.rectangle_container = self.image_canvas.create_rectangle(
            0, 0, TAG_RECT_SIZE, TAG_RECT_SIZE, outline="white", width=3
        )
    ...
    def load_tag_data(self):
        ...
        tag_data = fops.load_tag_data(
            self.input_files[self.current_image_index][1].split(".")[0]
        )
        if tag_data is not None:
            ...
            for tag in tags:
                ...
                tags_string += tag["tag"] if tags_string == "" else f", {tag['tag']}"
    ...
    def get_rect_half_size(self):
        """Return the half-size of the tag rectangle (fixed size)."""
        return TAG_RECT_SIZE / 2, TAG_RECT_SIZE / 2
